...
int main(int argc, char *argv[]) {
    ...
    if (argc < 3) { fprintf(stderr, "Usage: ./grep searchre file(s)\n"); exit(ARGC_ERROR);}
    else { zero = (unsigned *)malloc(nlall * sizeof(unsigned));  tfname = mktemp(tmpXXXXX);  init(); const char* search_for = argv[1]; process_dir(argv[2], search_for, search_file);}
    printf("\n");
    printf("\nquitting...\n");
    exit(1);
}
...
int advance(char *lp, char *ep) {
  ...
  for (;;) {
    switch (*ep++) {
      ...
      case CEOF:  loc2 = lp;
            return(1);
      case CCL:   if (cclass(ep, *lp++, 1)) {  ep += *ep;  continue; }
            return(0);
      case NCCL:  if (cclass(ep, *lp++, 0)) { ep += *ep;  continue; }
            return(0);
...
void compile(int eof) {
  ...
  for (;;) {
    if (ep >= &expbuf[ESIZE]) { expbuf[0] = 0;  nbra = 0;  error(Q); }
      ...
      if (c == '\n') { peekc = c;  c = eof; }
    if (c==eof) {
        if (bracketp != bracket) { expbuf[0] = 0;  nbra = 0;  error(Q); }
        *ep++ = CEOF;
        return; }
    if (c!='*') { lastep = ep; }
    switch (c) {
      ...
      case '\n':
            ...
            error(Q);
      case '*':
            if (lastep==0 || *lastep==CBRA || *lastep==CKET) { *ep++ = CCHR;  *ep++ = c; }
            *lastep |= STAR;
            continue;
      case '$':
...
void onhup(int n) {
  ...
  if (dol > zero) {  addr1 = zero+1;  addr2 = dol;  io = creat("ed.hup", 0600);
      if (io > 0) { putfile(); } }
  fchange = 0;
    quit(0);
}
...
void putfile(void) {
    unsigned int *a1;
    char *fp, *lp;
    int n, nib = BLKSIZE;  fp = genbuf;  a1 = addr1;
  do {
    lp = getline_blk(*a1++);
    for (;;) {
      if (--nib < 0) {
